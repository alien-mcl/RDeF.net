...
namespace Given_instance_of.AttributeMappingSource_class
{
    ...
    public class when_gathering_entity_mappings : AttributeMappingSourceTest
    {
        ...
        {
            Result.OfType<IEntityMappingProvider>().Should().HaveCount(1).And.Subject.First().GetTerm(Array.Empty<QIriMapping>()).ToString().Should().Be("Product");
        }
        ...
        {
            Result.OfType<IPropertyMappingProvider>().All(property => property.Property.Name.ToLower() == property.GetTerm(Array.Empty<QIriMapping>()).ToString()).Should().BeTrue();
        }
    }
}
