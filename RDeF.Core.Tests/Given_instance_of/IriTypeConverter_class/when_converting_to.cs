...
namespace Given_instance_of.IriTypeConverter_class
{
    ...
    public class when_converting_to : IriTypeConverterTest
    {
        ...
        {
            Converter.ConvertTo(new Iri("Test"), typeof(string)).Should().Be("Test");
        }

        [Test]
        public void Should_provide_an_original_Uri_when_Iri_was_created_from_Uri()
        {
            Converter.ConvertTo(new Iri(Uri = new Uri("Test", UriKind.Relative)), typeof(Uri)).Should().Be(Uri);
        }

        [Test]
        public void Should_convert_to_Uri()
        {
            Converter.ConvertTo(new Iri("Test"), typeof(Uri)).Should().BeOfType<Uri>().Which.ToString().Should().Be("Test");
        }
